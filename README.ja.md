      ...
      - **存在結果(実験08、2026-07-11)**: 構成的に物理な ρ=BB† を実装
        ...
        - **squeezed-product ansatz + #28 スコープ判定(族の適応力、実験11、2026-07-13 実測)**: 多モード
          ...
          **実験19 で判定(issue #38、2026-07-16)**: 純検出 ket 族は各自の rank 上限にほぼ到達
          ...
          薄い: 族はターゲットを採点最大カットオフ(n=12)で 1−F ≈ (2–3)×10⁻³ まで
          近似できる(1 モードの best-found 値、カットオフとともになお微増中 = 真の
          距離の上界)ため、
          実験19 の blind 残差 0.077 は族境界でなくフィット・データ予算由来。
          **頑健性スイープ(実験21、issue #67、2026-07-19)**: 3 data seed × 雑音強度 4 倍幅の
          全 5 構成で事前宣言した blind 判定が維持(lossy 代表 0.893–0.949 vs MLE 0.815–0.936、
          15 フィット中崩壊 basin ゼロ)。ただし MLE 対戦相手は実験19 と同じ 900 秒予算で、
          収束判定に達したのは 5 構成中 2 構成のみ — 示せたのは「同一予算のフルランク MLE に
          対する頑健性」であり、未収束 3 構成に対するマージンが MLE の最適化継続後も残る
          ことまでは主張しない(依然 1 ターゲットクラス・探索的、普遍主張は不可)。
          **splat の score 1.7674 (>1) は非物理性の直接証明**(純粋ターゲットとの tr(ρσ)≤1 は
          物理状態のみ)— issue #8 の tension がヘッドライン数値に露出した形。
          残: なし(σ_add/シード sweep は実験21、非包含テストは実験20、held-out フルランク・
