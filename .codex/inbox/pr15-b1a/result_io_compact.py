...
def _validate_provenance(record, where):
    ...
    if (
        ...
        raise ResultRegistryError(
            f"{where}.provenance retained flags must be booleans"
        )
    if not _text(provenance["absence_note"]):
        raise ResultRegistryError(
            f"{where}.provenance.absence_note must be non-empty"
        )
    ...
    if evidence == "committed_raw_log":
        ...
        if provenance["raw_log_retained"] is not True:
            raise ResultRegistryError(
                f"{where} committed_raw_log requires raw_log_retained=true"
            )
        if not (
            isinstance(provenance["source_commit"], str)
            and COMMIT_RE.fullmatch(provenance["source_commit"])
        ):
            raise ResultRegistryError(
                f"{where}.provenance.source_commit must be a full SHA"
            )
        if not _text(provenance["artifact_path"]) or not _text(
            provenance["locator"]
        ):
            raise ResultRegistryError(
                f"{where} committed log path and locator must be non-empty"
            )
        return

    if evidence == "historical_report_only":
        ...
        if (
            ...
            raise ResultRegistryError(
                f"{where} historical evidence requires retained flags=false"
            )
        reported_in = provenance["reported_in"]
        if (
            not isinstance(reported_in, list)
            or not reported_in
            or any(not _text(source) for source in reported_in)
        ):
            raise ResultRegistryError(
                f"{where}.provenance.reported_in must be non-empty"
            )
        return
...
def validate_results(registry):
    ...
    for index, series in enumerate(figure["series"]):
        ...
        if series["id"] in series_by_id:
            raise ResultRegistryError(f"duplicate figure series id: {series['id']}")
        if not all(
            _text(series[field])
            for field in (
                "id",
        ...
        for field, expected in selector.items():
            if field not in primary or primary[field] != expected:
                raise ResultRegistryError(
