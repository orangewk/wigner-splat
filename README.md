...
**判定・3モード(2026-07-07、実測値。公式戦=実験06)**: **splat の両勝ち。** 3×3×3 角度組
× 2000 shots、bins=24(0.14 カウント/セル)にて — splat(6×6 完全共分散、28 パラメータ/個):
F 0.62–0.76(3シード)、稜線 (p₁+p₂+p₃)/√3 をデータから検出、負性回復、**~15 s**。
MLE(n_max=8 → **512 次元**、打ち切り上限 0.993): 0.72 s/反復、**900 s 予算で 935 反復・
F 0.701・未収束(DNF)**。対数尤度は ~40 反復で頭打ちになるが fidelity は 6×10⁻⁵/反復で
しか登らない — 測定制約 1.8万行 vs 密度行列 26万パラメータの劣決定性が原因で、正直な
外挿では収束まで **2時間超**(n_max=10 なら ×2.8)。スケーリング系列の確定値:
1モード 20次元/0.5–0.9 s(MLE 2倍速で勝ち)→ 2モード 144次元/~32 s(互角、splat 7.4倍速)
→ 3モード 512次元/**900 s DNF**(splat が fidelity・速度の両方で勝ち)。
**モード数 ≥ 3 では splat が実用上唯一のフル・トモグラフィー手段**、が本リポジトリの主結論。
...
- [x] 3モード拡張(実験06。splat F 0.62–0.76 / ~15 s vs MLE(512次元)F 0.701 / 900 s DNF
      — **初の両勝ち**。ビン平均補正の発見(セル中心値との比較はフリンジを系統的に減衰
      させるバイアス)込み。上記判定参照)
