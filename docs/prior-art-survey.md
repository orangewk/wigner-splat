...
- 追加者: Claude（研究指揮）。手法: web サーベイ 6 角度 + 一次ソース精読 + oracle（gpt-5.5）双方向 adversarial check → **Fable（claude）/ Sol（gpt-5.5）2 名の独立査読**で citation 精度・contingency 分離・Chabaud 特徴づけを修正。
...
| 論文(年) | 何をしたか | 系統 | 残る差分 |
|---|---|---|---|
| [Chabaud–Markham–Grosshans, Stellar representation of non-Gaussian quantum states](https://arxiv.org/abs/1907.11009) (PRL 2020) | Husimi/Bargmann 関数の零点で非ガウス性を階層化（stellar rank）。有限 stellar rank 状態 = **単一の** Gaussian ユニタリを**有限 Fock 重ね合わせ（core state = 次数 r の多項式 × Gaussian）に作用させた**状態 | 表現論 | **互いに包含しない別クラス**。BB† は**異なる** Gaussian パラメータの displaced-squeezed ket の重ね合わせで、これは一般に有限 stellar rank にならない（cat は Bargmann ∝ cosh で零点無限 → stellar rank ∞）。逆に有限 rank の core state は BB† の形（積 displaced-squeezed ket 和）に一般には乗らない。「BB† ＝ stellar/core-state 分解そのもの」は**誤り**。広義の「ガウス重ね合わせ」表現として隣接するのみ |
| [Marshall & Anand, Simulation of quantum optics by coherent state decomposition](https://arxiv.org/abs/2305.17099) (2023, Optica Quantum) | 有限ランク coherent 状態重ね合わせで量子光学を**シミュレート**（線形光学が rank 不変=free、m モードで複雑度 O(m²2ⁿ)、要一次確認） | 表現＋順問題 | 「有限 coherent 重ね合わせを**スケーラブルな表現**として使う」軸では **BB† にかなり近い**（弱い関連ではない）。分かつのは対象で、こちらは**順問題（回路シミュレーション・Boson sampling）**、BB† は測定からの**逆問題（トモグラフィ）**。splat 側の Kenfack/Tosca と同じ「表現は共有・逆問題は別」構造 |
...
| [Strandberg, Simple reliable noise-resilient CV QST with convex optimization](https://arxiv.org/abs/2202.11584) (PRApplied 2022) | homodyne/heterodyne から**凸最適化(SDP)**で Fock 基底密度行列を再構成。大域最適収束保証 | 逆問題（凸・物理制約） | ansatz は Fock 密度行列＋SDP、BB† は非凸の変分ガウス ket。**表現の近縁ではないが、CV トモグラフィ手法として正当なベースライン/比較対象**（＝ #31 での「few-parameter 変分 ansatz 系と近接」は誤特徴づけ、実体は SDP／Fock） |
| [Ohliger–Nesme–Gross–Liu–Eisert, CV compressed sensing / low-rank CV tomography](https://arxiv.org/abs/1111.0853) (2011–13) | CV/homodyne トモグラフィを**低ランク・構造事前**で少測定に定式化、標準法に優位 | 逆問題（構造事前・少データ） | 「構造事前で generic より少データ・過適合減」の柱を Tiunov より古く一般に先取り。BB† の"物理制約＝正則化"優位の**もう一つの既知系譜** |
| [Fedotova–…–Lvovsky, CV tomography of high-amplitude states](https://arxiv.org/abs/2212.07406) (arXiv 2022 / PRA 2023) | feed-forward NN で連続位置基底の密度行列を直接再構成。**Fock 打切りを回避**し高振幅へスケール | 逆問題（Fock-free スケーリング） | 「Fock-free で大きな状態へスケール」という**動機が同一**。ただし道具は NN（学習コスト大・ブラックボックス）、BB† は閉形式ガウス ket |
| [Heller, frozen Gaussians / coupled coherent states・vMCG](https://doi.org/10.1063/1.441588) (1981–、化学) | ガウス波束の**変分**重ね合わせで量子動力学を解く（40 年物の系譜） | 変分ガウス族（順問題・別分野） | 「ガウス ket 重ね合わせを変分フィットする」機構自体が Marshall–Anand より**さらに古い**先例。表現機構の非新規性の強い傍証（対象は分子動力学の順問題） |
...
**判定**: 「差分ゼロ」ではないが、**BB† の概念的支柱はほぼすべて先行に予約済み**。存在する novelty は**表現でも「物理が MLE に勝つ」洞察でもなく、アルゴリズム/実証**に限られる。ここで 2 つを分けて扱う（この分離を怠ると contingency を過剰一般化する）:
- **(i) Fock-free 微分可能推定器を"手法"として立てる**には、#27/#28 が既存手法（NN/凸/Fock-MLE）に**勝つ領域を実証**して初めて論拠が立つ（contingent）。
- **(ii) 物理 ansatz vs signed-splat（準確率）の対照実験**による物理性コスト/利得の切り分けは、**BB† が負けても成立する科学的貢献**（負け方の特徴づけ自体が結果。「ネガティブ結果がポジティブな出口を持つ」）。これは #27/#28 の勝敗に依存しない。
...
- **要追加精査**: Marshall & Anand の後継 [Fast simulations of CV circuits using coherent state decomposition](https://arxiv.org/abs/2508.06175) (2025)、Chabaud らの多モード stellar simulation、Řeháček/Hradil/Ježek 系の物理制約 MLE、**CV classical shadows**（Gandhari–Iosue–Albert 系 2022–24、Fock 打切りに依らないスケーラブル CV 学習のもう一つの路線）、**構造付き ansatz の効率学習**（Mele et al. 2024「few non-Gaussian gates で準備された状態の学習」）。「差分ゼロ」の最終確認は closeness≥6 候補への 3 票 adversarial 判定（§1 の手法）を BB† 系にも回すのが望ましい（今回は Fable+Sol の 2 票査読まで実施、独立精読は継続候補）。
