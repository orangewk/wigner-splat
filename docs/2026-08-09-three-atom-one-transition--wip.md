...
日付: 2026-08-09 / 著者: 本線 / status: **wip — v0.3: 保持枝の証明完成(r = 2 無条件 6100ε⁻⁴、
r = 1 二者択一 47ε⁻²/降下証明書)— 未レビュー。残 = 分裂枝の組み立てと降下 wrapper**
...
5. 合成: sup_K|G| ≤ 0.825·Q·|c₃|·(36ε^{−2}sup_E|η|)² = 0.825·1296·Q|c₃|ε^{−4}(sup_E|η|)²、
   sup_E|G| ≥ 0.175·Q|c₃|(sup_E|η|)²(pointwise 下界の sup)。比 ≤ 0.825·1296/0.175·ε^{−4}
   ≤ 6100ε^{−4}。∎
...
### 補題 L3a-hold(r = 1)【証明完了・未レビュー — 二者択一出力】

r = 1(M₀ = 0、M₁ ≢ 0)のとき、C₂₃ := |c₂| + |c₃|、σ₁ := sup_E|M₁| として、
**次のいずれかが成立**:
- (A) σ₁ ≥ 3C₂₃β²(床 active): **sup_K|G| ≤ 47 ε^{−2} sup_E|G|**。
- (B) σ₁ < 3C₂₃β²(床 inactive): **sup_K|c₂η + c₃μ| ≤ 108 C₂₃β²ε^{−2}** —
  「位相差の近共線性証明書」。これは有効 (2,1) 構造への遷移事象であり、降下 wrapper が
  受け取る(本補題の守備範囲外 — 一遷移の帳簿で処理)。
