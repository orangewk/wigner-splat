...
日付: 2026-08-09 / 著者: 本線 / status: **wip — v0.4.1: 分裂枝 = luna R2 反映済み(外側転送の
不等式本体は検算通過; blocking = s の存在・閉性 → 前提明示 + 閉性補題で修正、major = 座標統一・
P_B ≢ 0 分岐明示)。保持枝 = R1 通過済み。
残 = 証明書→wrapper の受け渡し契約(3 系統)、多重遷移**
...
## 3.7 降下 wrapper の設計ノート(v0.4.2 — 執筆中、Sol xhigh 協力並走)

目的: 証明書 3 系統(保持 r=1(B) / 分裂 (ii) / 分裂 (iii))を §4.3.5.3 の (W,d,Δ) 降下に
接続する契約の証明。以下は本線の予備解析 — 検証済み事実と open を区別して記す。

### 3.7.1 (B) 契約の核: 厳密恒等式によるカスケード【恒等式は検算済み】

κ := −c₂/c₃、e(t) := M₁(t)/c₃ と置くと μ = κη + e は**代数的に厳密**(r = 1 でも近似不要)。
代入すると(M₀ = 0 の下で)
  **G = c₃·g_κ(η) + c₃·e^{κη}·(e^{e} − 1)**(厳密恒等式 — 代数検算 + 数値 2000 配置
  相対誤差 ≤ 2·10⁻¹⁴、scratchpad wrapper_identity_check.py)。
含意: (B) の証明書(σ₁ = sup_E|M₁| 小)は E 上 |e| ≤ σ₁/|c₃| を与え、G は
**摂動付き g_κ 標準形**に厳密に還元される。r = 2 解析(§3.5 手順 2–4)の摂動版が回る:
- **床 active**: 0.175|c₃κ(κ−1)|(sup_E|η|)² が摂動項 |c₃|e^{|κη|}(e^{|e|}−1) に勝つ場合、
  E 側下界が立ち、K 側は既存の sup_K|G| ≤ 36ε⁻²σ₁ + 0.57C₂₃β² と合成して **ε⁻⁴ 転送で完結
  (降下不要 — (B) の大部分は wrapper に到達する前に自己解決する見込み)**。
  K 側で e を外挿しないこと(e^{|e|} は ε⁻²σ₁ 外挿で爆発)— K 側は M₁ 直接評価を使う。
- **床 inactive** のとき、負け方は 2 通りしかない:
  (a) |κ(κ−1)| 小 = κ ≈ 0, 1 = **係数退化** → 併合 slot(κ ≈ 0: c₂ ≈ 0 で実質 2 原子;
      κ ≈ 1: c₂ ≈ −c₃ で pair {2,3} の M₀ 縮退 — (iii) と同型)。
  (b) sup_E|η| 微小 = **原子 1, 2 の E 上衝突** → d↓ 併合 slot。
  いずれも (W,d,Δ) の離散降下に対応 — **カスケードは有限段で終端**。
- open: 閾値設計と定数、κ 爆発時(η 微小で |κ| 大)の swap 処理、(a)(b) の併合の正確な定義。

### 3.7.2 (iii) の表現剛性と包絡 discount(open — 設計方針のみ)

相異なる q_j の指数関数は線型独立なので **F の原子表現は一意** — η ≢ const のとき
「pair 併合」は F を変えない表現の取替えとしては存在しない。さらに反例の摂動
(q₂ = q₁ + 1/10 + δt²、c₁ = −c₂e^{1/10}、δ → 0)で U_F 付き転送の破綻は η 非定数でも
続く(比 ~ 1/δ)。従って (iii) の正しい処理は表現の取替えでなく、
**包絡の discount 降下**: U_F を諦め、導来 (2,1) 表現の包絡 U′ で kernel を主張し直す
(K2Q-wt は U′ に対して証明済み — 指数 4)。核心の open: 剰余 R = c₂(e^η−1−η) の
U′ 正規化は e^{−U′}|R| ~ c_Bβ_B²/|P_B| で P_B 深消滅時に非有界(ST1 型の罠の再来)。
§3.6.2 の天井 2 方式が U′ 正規化でどこまで延命するか、および (W,d,Δ) の deg ≤ 2(m_B−1)
制限と η² 項の帳簿整合が課題。

### 3.7.3 (ii) の構造解析(open)

分裂スケール I_s での一様深消滅 e^{−U}|F| < 2β_B² が強制する構造は未解析。
pair held + singleton 分離開始の境界で三包絡がほぼ水平に並ぶ必要があることまでは
定性的に言えるが、有効 valuation 上昇(Δ↓)か衝突(d↓)かの定量判定が必要。

### 3.7.4 状態機械の骨格(素案)

状態 = (表現, prepared chart, (W,d,Δ), 包絡 U_state)。遷移:
- 転送枝(保持 r=2 / r=1(A) / 分裂 (i)): 終端 — kernel 定数を消費して比を返す。
- (B) カスケード: 床 active → 終端 / κ 退化 → 併合(Δ↓ or d↓)/ η 衝突 → 併合(d↓)。
- (iii)・η ≡ const 併合: 包絡 discount + (2,1) 状態へ(W 3→(2,1) 帳簿、r_B metadata)。
- (ii): 未定(3.7.3 の解析待ち)。
well-foundedness: (W,d,Δ) 辞書順 + カスケード内の有限場合分け。包絡 discount の合成則
(降下 1 段あたりの定数損失と discount 量の帳簿)が §4.3.5.3 帰納への積み上げの残作業。
