...
日付: 2026-08-09 / 著者: 本線 / status: **wip — v0.5: 補題本体(転送 or 証明書)= 両枝レビュー
通過。wrapper は Sol 協力で問題が正確に分解された: 証明書だけからの strict 降下は不可能
(開条件 vs 閉代数条件)— 残る 2 補題 = ①c=3 quantitative preparation(主 blocker)
②exact pair-block kernel K(2)+1(修復候補)。(B) の (T) 部分枝は証明済みで自己解決。
残 = 上記 2 補題、多重遷移**
