...
日付: 2026-08-09 / 著者: 本線 / status: **wip — v0.6: 五次共鳴反例により Γ(3) = 4 目標は偽、
正しい sharp 予想は Γ(3) = 5**(反例は保持枝 r=1(B) 穴の内部 — 既証明の部分枝はすべて真の
まま)。v0.5: wrapper 分解(bridge/(DC)、残 2 補題)。K(2)+1 も目標 ρ⁻⁵(QR5)に変更。
残 = QR5、preparation lemma、多重遷移**
...
**訂正 2(v0.6)**: 指数 4 も**偽** — 五次共鳴反例
([K(2)+1 文書 §3.5](2026-08-09-pair-block-kernel-K2p1--wip.md))により **Γ(3) ≥ 5**。
反例 (c₁,c₂,c₃) = (1, −e^{iπ/3}, 1)(1 の原始 6 乗根係数)は**三原子 held(保持枝の内部)**で
比 ~ 0.998·ε⁻⁵(mpmath 検証)。既証明の部分枝(r=2: 8000ε⁻⁴ / r=1(A): 61ε⁻² /
分裂 (i): 100C₂₁s⁻⁴)は**そのまま真**(反例は r=1(B) 証明書穴に住む — (B) 穴が ε⁻⁵ を
含むことの証明になった)。補題全体の正しい目標指数は **Γ(3) = 5(予想)**。
...
**(T-4) 四次テスト**: Q_τ = (1 − e^{τt²})²(c = 3)。τ → 0 で τ⁻²Q_τ → t⁴、
包絡は平坦 ⇒ Γ(3) ≥ 4(K2Q の T1 と同型; Sol 検証済み)。
**(T-5) 五次共鳴テスト(v0.6 — 決定的)**: (1, −e^{iπ/3}, 1) 共鳴族
([K(2)+1 §3.5](2026-08-09-pair-block-kernel-K2p1--wip.md))⇒ **Γ(3) ≥ 5**。
3・4 次 jet の同時消滅は E = e^{±iπ/3}(原始 6 乗根)で非退化に起き、5 次は
G⁽⁵⁾ = ±2id⁵/√3 ≠ 0 で消せない ⇒ **Γ(3) = 5 が新しい sharp 予想**
(jet 分類上、非退化配置の接触次数は ≤ 5)。上界側の目標も ρ⁻⁵ に変更。
...
- v0.6(2026-08-09): **Γ(3) = 4 目標の反証と Γ(3) = 5 予想への移行**。五次共鳴反例
  ...
  §1 に訂正 2、§4 に (T-5) を追加。(B) 穴が ε⁻⁵ を含むことが確定 —
  wrapper/preparation の目標指数も 5 に連動変更。
