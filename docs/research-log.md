...
License Grant restrictions (non-commercial scientific research only; no
reproduction, modification, or third-party availability without prior written
permission) are both recorded without attempting to resolve the conflict.
...
**Happened.** The exp25 trefoil target got the exp28 treatment with
exact-enclosure inputs end to end (`experiments/29_trefoil_certified/`):
the target's squared amplitudes are rational, so every constant in the
margin formulas is bracketed by exact `fractions` arithmetic, and the
derivation memo also addresses the dictionary side for `K <= 2`
pure-Gaussian superpositions. What is claimed — the margin statements,
the winding bound, the stability-theorem variant, and the resulting
dictionary-gap statement — and each claim's status live only in that
memo's §8 claim table (W0 through W5, N1 through N5); this log restates
none of it.
...
fixed in place with correction notes in the memo, the referees replaced
by sound sample families, and the artifact regenerated by the corrected
evaluator. Referee tests (`tests/test_trefoil_certified.py`) re-prove
the enclosures in raw exact arithmetic, referee the margins on sound
sample families (including reviewer-style phase-normal probes), check
the zero-curve parametrization, and probe the winding bound on Gaussian
K=2 censuses; the claim-surface policy test scans this experiment's
