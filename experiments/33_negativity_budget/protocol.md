...
本packetが数値計算として対応する範囲は、承認済みstage 1の上端に合わせ
`0 <= beta <= 0.4` とする。この範囲では分母 `1-2 beta >= 0.2` である。
これは数値条件の契約であり、データが許す負性の物理的上限を事前決定するものではない。
...
2. 対応域上端`beta=0.4`を含め、各成分densityが正規化されている場合は
   signed densityも積分1になる。
3. 負densityを観測可能な形で保持し、strict NLLが拒否する。
4. beta domain、成分shape、mode不一致、非finite norm、非real/nonfinite観測を
   fail closedで拒否する。
