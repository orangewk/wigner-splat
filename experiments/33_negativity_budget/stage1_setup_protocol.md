...
同じthetaを重複groupとして渡さない。入力配列はcopyし、呼び出し後の外部mutationを
setupへ伝播させない。test dataを受け取る引数は設けない。
...
検査する。範囲と点数はtest sampleやfit結果で変更しない。group順とtheta順はinput順を保つ。
...
`w=1/4` としてphysical referenceを `rho_0=(1-w)A+wS` とし、初期実効rank 4を保つ。
...
- fresh modelをpackしたinitial parameter vector
...
4. beta>0でpositive/negative normが1、初期signed densityが独立構成したrank-4 physical
   referenceと一致する。
5. input mutationがsetupへ伝播せず、同じ `(train, beta, seed)` が同じsetupを返す。
6. beta domain、seed、configをfail closedで検証する。
