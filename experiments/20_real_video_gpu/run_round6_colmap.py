"""Run the Round 6 train-only COLMAP reconstruction for one public scene."""
...
ROUND6_DATA = HERE / "data" / "round6"
ROUND6_OUT = HERE / "out" / "round6"
...
def validate_train_boundary(scene: str) -> tuple[Path, list[str]]:
    scene_root = ROUND6_DATA / scene.lower()
    manifest = json.loads((scene_root / "manifest.json").read_text(encoding="utf-8"))
    if manifest.get("round") != 6 or manifest.get("scene") != scene:
...
def main() -> None:
    ...
    output = (
        ...
        else (ROUND6_OUT / args.scene.lower() / "colmap" / "exhaustive_v1").resolve()
    )
