...
Status: issue #137 Gate T′ sub-goal (a) slice 1, started 2026-08-02 under
orange's delegation of research direction (issue #137 phase-close comment).
This memo is written and committed **before** any implementation or run
output exists; the claims below are pre-declared, and the falsification
gates in §6 are fixed here. Measured numbers will live only in
...
`f_T|S3` (so `L_1(T)` is a smooth closed 1-manifold). Fix a tube radius
`rho > 0` such that the closed geodesic tubes `U_rho` of radius `rho`
around the components of `L_1(T)` in `S3_1` are pairwise disjoint embedded
solid tori. Define the two margins

```
m(rho)   = min over S3_1 \ U_rho of |f_T|                  (clearance),
sigma(rho) = min over U_rho of sigma_2( d(f_T|S3) )        (transversality),
```

`sigma_2` the smallest singular value of the real differential
`T_w S3 -> R^2`. Both are positive for regular targets and small `rho`.
...
`(S3_1, L_1(T))`. In particular component count, linking matrix, and
core windings (for tubes avoiding the cores) all coincide with the
target's.
...
```
eps0(T) = max over admissible rho, h > 0 of
          min( m(rho) * e^{-1/2},  sigma(rho) / c1(h) ),
```

and any `delta < eps0(T)` preserves the link type. `eps0` is a bound,
not an equality: the true breaking radius can only be larger.
...
- Clearance: outside both tubes `sin rho <= |w2| <= cos rho`, and
  `|f| = |w1||w2| = sin(theta)cos(theta)` is minimized at the tube
  boundaries, so `m(rho) = sin(rho)cos(rho) = sin(2 rho)/2`.
...
```
eps0(|11>) >= max over rho < pi/4, h > 0 of
              min( sin(2 rho) e^{-1/2} / 2,  cos(2 rho) h e^{-(1+h)^2/2} / sqrt(2) ),
```

a strictly positive explicit number. Its value, the optimizing
...
- **E2 (constant evaluation).** Optimize `(rho, h)` numerically for
  `eps0(|11>)`; record `eps0`, the fidelity form, and the optimizers.
  Alarm `F2`: `eps0 <= 0` or optimizer infeasibility — record, halt
  interpretation (no physics conclusion).
- **E3 (tightness probe; measured, not predicted).** Search for unit
  states at small `delta` whose census on the standard grid differs from
  the Hopf link; record the smallest breaking `delta_break` found and the
  ratio `delta_break / eps0`. Gate `F3` (blocking): any state with
  `delta < eps0` whose census differs from the Hopf link at census
  standards — theorem or census defect; halt all interpretation of this
  experiment and report before any further claim.
- **E4 (exp25 consistency).** Compare `1 - eps0`-fidelity bound of S5
  with the measured best-found fidelity of exp25's `|11>/D_coh/K=2` cell
  (read from `topological_kcurves.json`, never restated). Gate `F4`
  (blocking): measured best-found exceeds the S5 bound — contradiction
  between a proof and a measurement; halt and report.
...
| N3 | no census change inside `eps0`; `delta_break/eps0` recorded | numerical, E3 | run |
