...
def extract_empirical_upper_bound(route_b_path: Path) -> dict:
    ...
    return {
        ...
        "epistemic_status": "empirical best-found family-constrained approximation; not a certified lower bound and not an upper bound on K_epsilon^{G,F}",
        "r_epsilon_upper_points": {"claim": "At n_score=12, each fit demonstrates R_epsilon <= 2 at its listed epsilon; K only varies the ket components per rank-2 column.", "points": n12_points},
        "certified_lower_bound_reservation": {"status": "reserved_not_computed", "scope": "Gate T certified lower bounds"},
...
def extract_data_proxy(exp18_path: Path, exp14_log_path: Path) -> dict:
    ...
    return {
        ...
        "epistemic_status": "data proxy only; not K_epsilon^{G,F} and not a certified lower bound",
        "certified_lower_bound_reservation": {"status": "reserved_not_computed", "scope": "Gate T certified lower bounds"},
