...
JSON retains sampled and certified columns: `N_robust_sampled` /
`N_robust_certified` for `psi_trunc`, and their `N_robust_lifted_*`
counterparts for the untruncated finite-energy comb. The certified count uses
...
circle minimum. Lifting adds `sqrt(fock_tail_upper_bound)` and
`lattice_envelope_amplitude_bound` to the Rouché perturbation threshold. At
`delta=0.18` in the largest window, the certified ideal-comb count remains
nonzero for `Delta=0.3, epsilon=1e-4` and `Delta=0.4, epsilon<=1e-3`;
`Delta=0.2` remains certified only for `psi_trunc`.
...
column `N_robust_certified/C - B(R+delta)`.  It deliberately does not insert a guessed
...
`N_robust_certified` is unchanged when `n_max` rises from 160 to 200, cross-checks roots by
