...
def certification_from_rows(rows: list[dict], max_radius: float, delta: float) -> dict:
    """Derive configuration-level scope and prose solely from certified counts."""
    lifted = {
        row["epsilon"]: row["N_robust_lifted_certified"]
        for row in rows
    ...
    certification = {
        "ideal_comb": {
            ...
            "max_N_robust_lifted_certified": max(lifted.values()),
        },
    ...
    if nonzero_epsilons:
        certification["description"] = (
            ...
            f"{_format_epsilon(max(nonzero_epsilons))} at delta={delta}, with maximum "
            f"N_robust_lifted_certified={certification['ideal_comb']['max_N_robust_lifted_certified']}."
        )
    else:
        certification["description"] = (
            "Only psi_trunc is certified at this cutoff: all scanned "
            f"lifted certified counts are {certification['ideal_comb']['max_N_robust_lifted_certified']} at delta={delta}."
        )
        ...
        for radius in radii:
            ...
            for disk_radius in DISK_RADII:
                ...
                for epsilon in EPSILONS:
                    ...
                    certified_count = sum(row["certified_robust"] for row in robust_rows if row["epsilon"] == epsilon)
                    sampled_lifted_count = sum(row["sampled_lifted_robust"] for row in robust_rows if row["epsilon"] == epsilon)
                    certified_lifted_count = sum(row["certified_lifted_robust"] for row in robust_rows if row["epsilon"] == epsilon)
                    elevated_count = sum(row["certified_robust"] for row in elevated_rows if row["epsilon"] == epsilon)
                    assert certified_count <= sampled_count
                    assert certified_lifted_count <= sampled_lifted_count
                    assert certified_count == elevated_count, (envelope_delta, radius, disk_radius, epsilon, certified_count, elevated_count)
                    rows.append({
                        "R": radius,
                        "delta": disk_radius,
                        "epsilon": epsilon,
                        ...
                        "N_robust_certified": certified_count,
                        "N_robust_lifted_sampled": sampled_lifted_count,
                        "N_robust_lifted_certified": certified_lifted_count,
                        "K_lower_bound_symbolic": f"{certified_count}/C - B*({radius + disk_radius:.3f})",
                    })
...
def make_figure(result: dict, output: Path) -> None:
    ...
    for ax, (envelope_delta, roots, rows) in zip(axes, result["map_data"]):
        robust = {(round(row["zero"][0], 12), round(row["zero"][1], 12)) for row in rows if row["epsilon"] == 1e-3 and row["certified_robust"]}
...
def main() -> None:
    ...
    payload = {
        "schema_version": 3,
        "epistemic_status": "Rouche verdicts use a coefficient-norm derivative interval lower bound, not a sampled circle minimum; sampled and certified counts are both retained.",
        "lifting_rule": (
            "For a truncated-state zero disk, ||psi_trunc - phi|| <= sqrt(fock_tail_upper_bound) + lattice_envelope_amplitude_bound + d(epsilon). "
            "Therefore the disk lifts to the untruncated finite-energy comb when circle_minimum_certified_lower_bound exceeds "
        ...
        "theorem_scope": "The symbolic bound is K >= N_robust_certified/C - B(R+delta); C is intentionally not assigned a numerical value.",
        ...
        "truncation_protocol": "componentwise resolved Fock tail plus exact second-factorial-moment Markov remainder; N_robust_certified asserted invariant when n_max rises from 160 to 200",
